...
class FlameParamDiffusionModel(BaseModel):
    ...
    def decode_pretrained(self, codedict,image_name=None):
        ...
        print("identity_code.shape", identity_code.shape)
        batch_size = identity_code.shape[0]
        ...
        save_dir = os.path.join(self.cfg.dataset.identity_folder_name, image_name[0], 'mesh')
        print(f"save_dir: {save_dir}")
        os.makedirs(save_dir, exist_ok=True)
        
        # Save the mesh as PLY file
        save_path = os.path.join(save_dir, f"{image_name[0]}.ply")
        
        # Convert torch tensor to numpy array
        mesh_vertices = pred_mesh[0].detach().cpu().numpy()
