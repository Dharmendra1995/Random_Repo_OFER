...
class FlameParamDiffusionModel(BaseModel):
    ...
    def decode_pretrained(self, codedict,image_name=None):
        ...
        print(shapecode.shape, 'shapecode')


        with torch.no_grad():
            if self.with_exp:
                pred_flameparam = self.diffusion.sample(num_points=self.cfg.net.flame_dim, context=identity_code, batch_size=identity_code.shape[0], sampling=self.cfg.model.sampling, shapeparam=shapecode)
                print(pred_flameparam.shape, 'pred_flameparam') 
                if self.cfg.net.flame_dim == 53:
                    pred_expparam = pred_flameparam[:,:50]
                    pred_jawparam = pred_flameparam[:,50:] * 0.5
                    pred_mesh, pred_lmk2d, pred_lmk3d = self.flame(shape_params=shapecode, expression_params=pred_expparam.float(), jaw_params=pred_jawparam.float()) 
                    print(pred_mesh.shape, 'pred_mesh')

        for i in range(batch_size):
            print(batch_size, 'batch_size')
            print(image_name, 'image_name')

            # Create the directory structure: parent_folder/image_name
            save_dir = os.path.join(self.cfg.dataset.identity_folder_name, image_name[i], 'mesh')
            print(f"save_dir: {save_dir}")
            os.makedirs(save_dir, exist_ok=True)
            
            # Save the mesh as PLY file
            save_path = os.path.join(save_dir, f"{image_name[i]}.ply")
            
            # Convert torch tensor to numpy array
            mesh_vertices = pred_mesh[i].detach().cpu().numpy()
            
            # Create a trimesh object with the vertices
            # Assuming FLAME topology is used, which has a fixed face structure
            # If you don't have faces information, you might need to add it here
            # mesh = trimesh.Trimesh(vertices=mesh_vertices, process=False)
            trimesh.Trimesh(vertices=mesh_vertices * 1000.0, faces = self.flame.faces_tensor.cpu().numpy(), process=False).export(f'{save_path}')
            
            # Export the mesh as PLY
            # mesh.export(save_path)
            
            logger.info(f"[{self.tag}] Saved mesh to {save_path}")
